...
@login_required
def seller_bulk_accept_orders(request):
    from django.contrib import messages
    from django.utils import timezone
    ...
    accepted_count = orders.update(
        status="CONFIRMED",
        seller_accepted_at=timezone.now(),
    ...
    if accepted_count:
        messages.success(
            request,
            f"{accepted_count} pending order(s) accepted successfully."
        )
    else:
        messages.info(
            request,
            "There are no pending orders to accept."
        )
...
def seller_dashboard(request):
    ...
    from django.utils import timezone as _dashboard_timezone

    today = _dashboard_timezone.localdate()

    today_orders = seller_orders.filter(
        created_at__date=today
    )

    today_pending_orders = today_orders.filter(
        status="PENDING"
    ).count()
